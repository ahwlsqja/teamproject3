...
// 펫 생성 API
router.post("/create", uploadPetImage, petController.createPet);

// 펫 1마리 조회하기 API(유저입장)
router.get("/findonly/:petId", authMiddleware, petController.findOnePet);

// 유저의 모든펫 조회하기 (제 3자 입장에서)
router.get("/findall/:userId", petController.findUserPets);

// 펫 정보 수정하기 (유저 입장)
router.put("/change/:petId", authMiddleware, uploadPetImage, petController.updatePetInfo);

// 펫 정보 삭제하기 (유저 입장)
router.delete("/delete/:petId", authMiddleware,  petController.deletePetInfo);

// 종류별 펫 찾기 (제 3자 입장)
router.get("/findfromtype/:petType", petController.findPetByPetType)
