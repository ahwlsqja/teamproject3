export class Userscontroller {
  ...
  verifySignUp = async (req, res, next) => {
    try {
      ...
      const pass_data = await this.usersService.verifySignUp(
        email,
        verifiedusertoken
      );
      return res
        .status(200)
        .json({ message: "인증에 성공했습니다.", data: pass_data });
    } catch (err) {
      next(err);
    }
  };
  ...
  updateUserInfo = async (req, res, next) => {
    try {
      const { email, password, name, phone_Number, intro, age, gender } =
        req.body;
      const imageUrl = req.file.Location;
      if (!email || !password) {
        return res
          .status(400)
          .json({ message: "이메일, 비밀번호를 입력해주세요." });
      }
      if (!imageUrl) {
